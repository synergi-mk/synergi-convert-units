...
var measures = {
  ...
  'Relative Concentration': require('./definitions/relativeConcentration'),
  'Absolute Concentration': require('./definitions/absoluteConcentration')
};
...
Converter.prototype.checkConvertUnits = function (unit1, unit2) {
  var found = false;
  var categories1=this.getAllUnitMeasurement(unit1);
  var categories2=this.getAllUnitMeasurement(unit2);
return categories1.some(element => {
  return categories2.includes(element);
});
};
Converter.prototype.getAllUnitMeasurement = function (checkUnit) {
  var found = [];
  ...
  each(measures, function (systems, measure) {
    ...
    each(systems, function (units, system) {
      ...
      each(units, function (unit, testAbbr) {
        if (testAbbr.localeCompare(checkUnit) === 0) {
          found.push(measure)
        }
      });
    });
