var convert
  ...
  , measures = {
    ...
    , relativeHumidity: require('./definitions/relativeHumidity')
    , torque: require('./definitions/torque')
    , gaugePressure: require('./definitions/gaugePressure')
    , currency: require('./definitions/currency')
    , percentage: require('./definitions/percentage')
    , pulseFrequency: require('./definitions/pulseFrequency')
  }
  , Converter;
...
Converter.prototype.toBest = function (options) {
  ...
  */
  each(this.possibilities(), function (possibility, system) {
...
Converter.prototype.possibility = function (originSystem) {
  ...
  each(measures[measure], function (units, system) {
    ...
    if (system == '_anchors')
      return false;
    if (!originSystem || system === originSystem)
      possibilities = possibilities.concat(keys(units));
  });
  if (possibilities.length === 0)
    possibilities = possibilities.concat(this.origin.abbr);
