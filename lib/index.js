...
var Converter = function (numerator, denominator) {
  if (denominator) this.val = (numerator || 0) / denominator;
  else this.val = numerator || 0;
};
...
Converter.prototype.measures = function () {
  return keys(measures).concat(keys(alias));
};
...
Converter.prototype.unitsFromMeasure = function (measure) {
  return measures[measure] || alias[measure];
};
  ...
  each(measures, function (systems, measure) {
    each(systems, function (units, system) {
      ...
      each(units, function (unit, abbr) {
        ...
        if (rec) throw new Error('Duplicate unit: ' + abbr + ' in ' + rec.measure);
